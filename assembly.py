...
def parse_arguments(args, log):
	...
	parser.add_argument("-o", "--outdir", dest = "output_dir",
		action = "store", default = None, type = str,
		help = "Location of output directory (default=indir)",
		required = True)
...
def trim_reads(R1_file, R2_file, output_dir, threads, log):
	# Running the actual trimming for each sample using enre-filter
	log.info("Trimming reads...")
	prefix = R1_file.split("_R1")[0].replace(".","_").replace(" ","_")+"_trimmed"
	os.system("erne-filter --query1 s% --query2 %s --output-prefix %s --q 20 --threads %s"
		%s (R1_file, R2_file, output_dir+"/"+prefix, str(threads)))
	return prefix
...
def assemble_reads(prefix, input_dir, temp_dir, output_dir, logs_dir, threads, ram, log):
	...
	files = list_directory(input_dir, 'files', 1):
		if prefix+"_trimmed_1.fastq" in files and prefix+"_trimmed_2.fastq" in files:
			# building SPAdes command
			spades_command = ("spades.py --pe1-1 %s --pe1-2 %s"
				%s (indir+"/"+prefix+"_trimmed_1.fastq"), indir+"/"+prefix+"_trimmed_2.fastq")
			# include orphaned sequences if available
			if prefix+"_unpaired.fastq" in files:
				spades_command += (" --pe1-s "+indir+"/"+prefix+"_trimmed_unpared.fastq")
			spades_command += (" -o %s --threads%s --memory %s" %s (temp_dir, str(threads), str(ram)))
			# run SPAdes
...
def main():
	...
	# iterate over filepairs
        for file in filelist:
			...
			R1_file = file
			R2_file = file.replace('_R1', '_R2')
			log.info("processing filepair:\t"+R1_file+",\t"+R2_file)
			# create a fastCQ quality report
			os.system("fastqc "+R1_file+" "+R2_file+" -o "+outdir+"/quality_control_reads")
			# Perform quality trimming the raw sequences
			erne_out = outdir+"/temp/trimmed_reads"
			prefix = trim_reads(R1_file, R2_file, erne_out, args.threads, log)
	...
	files = list_directory(outdir+"/quality_control_reads"), "files", 1)
	for file in files:
		if file.endswith(".zip"):
			os.system("unzip "+outdir+"/quality_control_reads"+file
				+" -d "+outdir+"/quality_control_reads"+file.replace('.zip', ''))
	# create multiqc report
	os.system("multiqc "+outdir+"/quality_control_reads"
